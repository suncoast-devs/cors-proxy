...
function getHandler(options, proxy) {
  ...
  return function (req, res) {
    ...
    if (!urlPart.startsWith(secretToken)) {
      res.writeHead(400, { 'X-Reason': 'Invalid secret token' })
      res.end('Invalid secret token')
      return
    }

    // Get the remaining part
    var location = parseURL(urlPart.slice(secretToken.length))
